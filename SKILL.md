---
name: vibe-spec
description: 为 Vibe Coding 项目建立可追溯、经确认的协作契约。用于项目仓库的新建、修改、规划、排障、文档、测试、打包和交付；尤其适合需要需求台账、数据设计确认、进度治理与安全 Git 范围的项目。
---
...
1. 从仓库根目录向上读取适用的 `AGENTS.md` 和 `CLAUDE.md`，检查仓库内的 `README`、`docs/`、需求、设计、进度、测试、迁移与构建文件。
...
4. 没有可用等价物时，从本 Skill 安装目录（即本文件所在目录）下的 [assets/governance-starter](assets/governance-starter) 复制所需模板，并替换占位内容。
5. 创建或更新仓库根目录的 `DOCUMENT_MAP.md`。它是职责到实际路径的唯一索引，必须写出每个文档或目录的真实位置；只用于导航，不承载产品或技术事实，也不覆盖项目既有文档优先级。
6. 创建或更新仓库根目录的 `AGENTS.md`：保留已有项目规则，补入文档优先级和 `DOCUMENT_MAP.md` 的路径。`AGENTS.md` 只写行为和索引，不复制产品、数据库或实现细节。仓库已有或需要 `CLAUDE.md` 时，不在其中复制规则，只写一行指针 `@AGENTS.md`（Claude Code 的导入语法）；协作规则始终只维护 `AGENTS.md` 一份。
...
1. 阅读根目录 `AGENTS.md`（经 `CLAUDE.md` 指针引用时同效）、`DOCUMENT_MAP.md`、相关需求/决策/进度/产品/技术/UI/业务流程文档，以及受影响实现、测试、配置、迁移和构建脚本。
...
4. 判断是否属于跨模块变更。同一持久化资源有多个写入入口或读取方，状态跨 API、SSE、确认卡、缓存或后台任务传播，涉及 Agent、检索、索引、图片、文件、Skill、MCP 等，或涉及 Web/桌面端/部署运行时、权限、密钥时，都必须按跨模块处理。
...
- 不得植入隐藏水印、隐藏标记、病毒、后门或任何难以发现的恶意逻辑；交付物中的行为必须对用户可见、可审查。
