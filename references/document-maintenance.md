...
`DOCUMENT_MAP.md` 是模板职责与项目实际路径之间的唯一索引。项目可能已有自己的需求文档、PRD 文档或开发计划文档（如 `docs/REQUIREMENTS.md`、`docs/PRD.md`、`docs/PLAN.md`），命名与路径以项目实际为准；实施和交付一律引用映射后的真实路径。
...
满足下列任一条件时，非 Bug 需求必须创建详细需求记录：同一持久化资源有多个写入入口或读取方；状态跨 API、SSE、确认卡、缓存或后台任务传播；涉及 Agent、检索、索引、图片、文件、Skill、MCP 等；涉及多个运行时或平台、权限、密钥。
